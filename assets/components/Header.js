			...
			<View style={globalStyles.headerBody}>
				...
				{gardenAddress && route.name === 'my-garden' && (
					<Text style={[globalStyles.body, { fontSize: fontSizes.body }]}>
						{lang.myGarden.default.meetsAt} <Text style={globalStyles.fontBold}>{gardenAddress}</Text>
						{gardenDaysTimes && (
							<Text>
								{' '}
								{lang.myGarden.default.on} <Text style={globalStyles.fontBold}>{formatGardenSchedule(gardenDaysTimes)}</Text>
							</Text>
						)}
					</Text>
				)}
